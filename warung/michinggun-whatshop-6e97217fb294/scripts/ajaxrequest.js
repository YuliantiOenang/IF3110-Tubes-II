	...
	this.loadXMLDoc=function(url,params){
		...
		if(req){
			...
			if(method=="POST"){
				...
				if(params)
					url+="?"+params;req.open(method,url,true);
				req.send(null);
			}
			return true;
		}
	...
	var processReqChange=function()
	{
		if(req.readyState==4&&req.status==200){
			...
			for(var i=0;i<commands.length;i++){
				method=commands[i].getAttribute('method');switch(method)
				{
					case 'alert':
					...
					case'setproperty':
						var target=getNodeValue(commands[i],'target');
						var property=getNodeValue(commands[i],'property');var value=getNodeValue(commands[i],'value');
