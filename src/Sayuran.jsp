...
<body>
	...
	<div id="container">
		...
		<!-- Navbar Section -->
		<p id="bbuy-a"> Products in "Sayuran" Category</p>
		<form method=post action="getsayuran">
			<select name="sort">
		...
		<div id="dagangan">
			...
			out.print((Integer)session.getAttribute("syjumlah"+i));%>
				<div class="kolom-6 product">					
					<div class="frame">
						<div class="kolom-6">
							<img class="gambar" src="res/img/product/<%str =(String)session.getAttribute("synama"+i);out.print(str);%>.jpg" alt=""/>
						</div>
						<div class="kolom-6">
							<p class="nama-produk-b"><a href="getbarang?namabarang=<%=(String)session.getAttribute("synama"+i)%>"><%str =(String)session.getAttribute("synama"+i);out.print(str);%></a></p>
							<p class="harga">Harga: <% int intr =(Integer)session.getAttribute("syharga"+i);out.print(intr);%> /kg</p>
							
							<div class="frame buy-bar">
								<form action="CartController" method="post" onsubmit="return validatejumlah('<%out.print((String)session.getAttribute("synama"+i));%>','<%out.print("cartJumProduk"+i);%>')" accept-charset='UTF-8'>
								<input class="kolom-9 buy-box" type="text" id='<%str =(String)session.getAttribute("synama"+i);out.print(str);%>' name="cartQuantity" value="Banyaknya barang.." onfocus="checkclear(this)" onblur="checkempty(this, 'Banyaknya barang..')"> 
								<input type="hidden" name="action" value="add">
								<input type="hidden" value="getsayuran" name="pageasal">
								<input type="hidden" value="" name="cartDetail">
								<input type="hidden" value="<%out.print((String)session.getAttribute("synama"+i));%>" name="cartNama">
								<input type="hidden" value="<%out.print((Integer)session.getAttribute("syharga"+i));%>" name="cartHarga">
								<input type="hidden" value="<%out.print((Integer)session.getAttribute("syjumlah"+i));%>" id="<%out.print("cartJumProduk"+i);%>" name="cartJumProduk">
								<input type="hidden" value="<%out.print((String)session.getAttribute("sykategori"+i));%>" name="cartKategori">
		...
		<div id="pagination">
			<ul>
				<%String n = ""+(Integer.parseInt(request.getParameter("f")));
				 ...
				 <% }
				 if (Integer.parseInt(request.getParameter("l"))<(Integer)session.getAttribute("jumlahsayuran")){
